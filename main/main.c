...
#include "../components/dir_sensor/dir_sensor.h"
...
void app_main(void){
    vector3_t v_accel,v_gyro,v_mag;
    init_i2c_dir_sensor(GPIO_NUM_22, GPIO_NUM_21);
    while(1){
        get_vector3(&v_accel, &v_gyro, &v_mag);
        ESP_LOGI("main", "Accel->x: %2.3f, ->y: %2.3f, ->z: %2.3f ", v_accel.x, v_accel.y, v_accel.z);
